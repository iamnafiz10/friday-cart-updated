...
function ShopContent() {
    ...
    // Redux products
    const productsFromRedux = useSelector((state) => state.product?.list || []);
    ...
    // Sync local products with Redux
    useEffect(() => {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (dropdownRef.current && !dropdownRef.current.contains(e.target)) setFilterOpen(false);
        };
                        ...
                        {filterOpen && (
                            <div
                                ...
                                <button onClick={() => handleCategoryChange("")}
                                        className={`block w-full text-left px-4 py-2 text-sm hover:bg-slate-50 ${selectedCategory === "" ? "text-slate-800 font-semibold" : "text-slate-600"}`}>
                                    All Products
                                </button>
                                {categories.map(cat => (
                                    <button key={cat.id} onClick={() => handleCategoryChange(cat.name)}
                                            className={`block w-full text-left px-4 py-2 text-sm hover:bg-slate-50 ${selectedCategory === cat.name ? "text-slate-800 font-semibold" : "text-slate-600"}`}>
                                        {cat.name}
                                    </button>
                                ))}
                            </div>
                                ...
                                {Array.from({length: totalPages}).map((_, index) => {
                                    ...
                                    return (
                                        <button key={pageNum} onClick={() => handlePageChange(pageNum)}
                                                className={`px-4 py-2 rounded-md border text-sm ${currentPage === pageNum ? "bg-slate-700 text-white border-slate-700" : "bg-white text-slate-600 border-slate-300 hover:bg-slate-100"}`}>
                                            {pageNum}
                                        </button>
                                    );
                                })}
